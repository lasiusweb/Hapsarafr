        ...
        <div className="p-6 bg-gray-50 min-h-full">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="bg-white rounded-lg shadow-xl p-8">
                    {!image && !isCameraOn && (
                         <div className="text-center p-8 border-2 border-dashed border-gray-300 rounded-lg">
                            ...
                            <div className="flex justify-center gap-4">
                                <button onClick={() => fileInputRef.current?.click()} className="px-6 py-3 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700 transition">
                                    Upload Image
                                </button>
                                <button onClick={handleUseCamera} className="px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition">
                                    Use Camera
                                </button>
                            </div>
                        </div>
                    ...
                    {image && !isCameraOn && (
                        <div className="mt-6">
                            <h2 className="text-xl font-bold text-gray-800 mb-4">Image Preview & Analysis</h2>
                            <div className="flex flex-col lg:flex-row gap-8">
                                <div className="lg:w-1/2 relative">
                                    <img src={image} alt="Crop preview" className="rounded-lg shadow-md w-full" />
                                     <button onClick={() => {setImage(null); setAnalysis(''); setError(null);}} className="absolute top-2 right-2 p-2 bg-black/50 text-white rounded-full hover:bg-black/75 transition" title="Clear image">
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg>
                                    </button>
                                </div>
                                <div className="lg:w-1/2">
                                    <button onClick={handleAnalyze} disabled={isLoading} className="w-full px-6 py-3 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700 transition disabled:bg-green-300 flex items-center justify-center gap-2">
                                        {isLoading ? (
                                    ...
                                    {analysis && (
                                        <div className="mt-6 p-4 bg-gray-50 rounded-lg border max-h-96 overflow-y-auto">
                                            <h3 className="text-lg font-bold text-gray-800 mb-2">AI Analysis</h3>
                                            <div className="prose prose-sm" dangerouslySetInnerHTML={{ __html: formatAnalysis(analysis) }} />
                                        </div>
                                    )}
                                </div>
                            </div>
