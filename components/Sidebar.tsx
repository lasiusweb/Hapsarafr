...
const Sidebar: React.FC<SidebarProps> = ({ view, onNavigate, currentUser, permissions, onSync, isSyncing, lastSync, isOnline }) => {
    ...
    const navItems = useMemo(() => [
        ...
        },
        {
            category: 'Community', items: [
                ...
                { view: 'mentorship', icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.653-.125-1.273-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.653.125-1.273.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm-9 5a2 2 0 11-4 0 2 2 0 014 0z" /></svg>, label: 'Mentorship' },
            ]
        },
        {
            category: 'AI Tools', items: [
                { view: 'crop-health', icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" /></svg>, label: 'Crop Scanner' },
                { view: 'yield-prediction', icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>, label: 'Yield Prediction' },
            ]
        },
        {
        ...
        },
        {
            category: 'Analytics', items: [
                ...
                { view: 'performance-analytics', icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>, label: 'Performance' },
            ]
        },
    ], [permissions]);
                                ...
                                {visibleItems.map(item => (
                                    <div key={item.view} className="relative group">
                                        <button
                                            ...
                                            className={`${itemClasses} ${view === item.view ? activeItemClasses : inactiveItemClasses} w-full`}
                                        >
                                            {item.icon}
                                            <span className={`ml-4 flex-1 text-left sidebar-item-text ${isCollapsed ? 'hidden' : ''}`}>{item.label}</span>
                                        </button>
                                         {isCollapsed && <div className="absolute left-full top-1/2 -translate-y-1/2 ml-3 px-2 py-1 bg-gray-900 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">{item.label}</div>}
                                    </div>
                                ))}
                            </div>
                        )
